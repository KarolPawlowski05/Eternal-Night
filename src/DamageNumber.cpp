...
DamageNumber::DamageNumber(float x, float y, int damage, bool isCrit, const sf::Font& font, bool isHeal)
    : GameObject(x, y), lifetime(MAX_LIFETIME), currentScale(1.f), isCrit(isCrit), isHeal(isHeal)
{
    // Krytyk - ciemnoczerwony i troche większy; zwykły - jasnoczerwony
    text.setFont(font);
    // Jeśli to leczenie, formatujemy tekst na zielono z plusem
    if (isHeal) {
        text.setString("+" + std::to_string(damage));
        text.setCharacterSize(20);
        text.setFillColor(sf::Color(50, 255, 50, 255)); // Jasny, zielony
        text.setOutlineColor(sf::Color(0, 80, 0, 200));  // Ciemnozielona obwódka
        text.setOutlineThickness(1.5f);
        vy = -60.f; // Leczenie unosi się delikatnie wolniej
    }
    // W przeciwnym razie robimy to, co do tej pory (zadawanie obrażeń)
    else{
        text.setString(std::to_string(damage));
